...
// Connect a 10K resistor from pin 2 (data) to pin 1 (power) of the sensor**************************************(Must do)
...
if(sensorValue>=400){
  ...
  float temp = analogRead( dht.temperature().getEvent(&event));
  ...
  // Get humidity event and print its value.
   float hum = analogRead(dht.humidity().getEvent(&event));
  if (isnan(hum)) {
